...
imu_correction = torch.tensor([0.0, 0.0, 0.0, 1.0])
...
class G1:
    def __init__(self, task="stand"):
        ...
        hip_pgain = 80.0
        hip_dgain = 2.0
        hip_roll_pgain = 120.0
        hip_roll_dgain = 3.0
        knee_pgain = 160.0
        knee_dgain = 4.0
        ankle_pgain = 40.0
        ankle_dgain = 1.0
        waist_yaw_pgain = 200.0
        waist_yaw_dgain = 5.0
        waist_pgain = 200.0
        waist_dgain = 5.0
        shoulder_pgain = 40.0
        ...
        elbow_pgain = 40.0
        elbow_dgain = 1
        wrist_roll_pgain = 20.0
        wrist_roll_dgain = 0.5
        wrist_pitch_pgain = 20.0
        wrist_pitch_dgain = 0.5
        wrist_yaw_pgain = 20.0
        wrist_yaw_dgain = 0.5
        ...
        (
            self.joint_limit_lo[free_limit_joints],
            self.joint_limit_hi[free_limit_joints],
        ) = (-100.0, 100.0)

        self.action_mask = np.ones(29)
        if LOCK_WAIST:
            self.action_mask[12:15] = 0.0
    ...
    def record_trajectory(
        self, dof_pos, dof_vel, base_ang_vel, projected_gravity, actions, task_obs
    ):
        current_obs_a = torch.cat(
            (dof_pos, dof_vel, base_ang_vel, projected_gravity, actions, task_obs),
            dim=0,
        ).numpy()
        self.trajectories[1 * LOG_STEP_LEN :] = self.trajectories[
            : -1 * LOG_STEP_LEN
        ].copy()
...
class DeployNode(Node):
    ...
    def __init__(self, task="stand"):
        ...
        shm_vprotate_quat = 4
        self.loc_shm_send = shared_memory.SharedMemory(
            create=True, size=np.prod(shm_send_shape) * 4
        )
        self.loc_shm_recv = shared_memory.SharedMemory(
            create=True, size=np.prod(shm_recv_shape) * 4
        )
        self.loc_shm_vprot = shared_memory.SharedMemory(
            create=True, size=np.prod(shm_vprotate_quat) * 4
        )
        self.loc_shm_vprot = shared_memory.SharedMemory(
            create=True, size=np.prod(shm_vprotate_quat) * 4
        )
        self.loc_send_data = np.ndarray(
            shm_send_shape, dtype=np.float32, buffer=self.loc_shm_send.buf
        )
        ...
        self.obs_quat[:] = np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32)
        self.loc_delta_rot = self.loc_send_data[7:11]
        self.loc_delta_rot[:] = np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32)
        self.obs_joint_pos = self.loc_send_data[11:]

        self.loc_delta_angle = 0.0

        self.joint_temperature = np.zeros((29))

        self.loc_recv_data = torch.frombuffer(
            self.loc_shm_recv.buf, dtype=torch.float32
        )
        ...
        if USE_DEX:
            self.dex_shm_send = shared_memory.SharedMemory(
                create=True, size=(75 * 2 + 7 * 2 + 1) * 4
            )
            self.dex_shm_send_data = np.ndarray(
                ((75 * 2 + 7 * 2 + 1),), dtype=np.float32, buffer=self.dex_shm_send.buf
            )
            self.dex_shm_recv = shared_memory.SharedMemory(create=True, size=14 * 4)
            self.dex_shm_recv_data = np.ndarray(
                (14,), dtype=np.float32, buffer=self.dex_shm_recv.buf
            )

        self.loc_process = Process(
            target=localization.start_service,
            args=[
                self.loc_shm_send.name,
                self.loc_shm_recv.name,
                self.loc_shm_vprot.name,
            ],
        )
        ...
        self.vp_delta_rot_mat[0, 0] = 1.0
        self.vp_delta_rot_mat[1, 1] = 1.0
        self.vp_delta_rot_mat[2, 2] = 1.0
        self.vp_delta_rot_mat[3, 3] = 1.0

        self.head_rot[-1] = 1.0
        self.left_hand_rot[-1] = 1.0
        self.right_hand_rot[-1] = 1.0
        self.vp_delta_rot[-1] = 1.0

        # cmd and observation
        # self.xyyaw_command = np.array([0, 0., 0.], dtype= np.float32)
        self.xyyaw_command = np.array([0, 0.0, 0.0], dtype=np.float32)
        self.commands_scale = np.array(
            [self.env.scale_lin_vel, self.env.scale_lin_vel, self.env.scale_ang_vel]
        )
    ...
    def launch_dex_hands(self):
        self.dex_process = multiprocessing.get_context("spawn").Process(
            target=start_dex_server,
            args=[self.dex_shm_send.name, self.dex_shm_recv.name],
        )
    ...
    def reset_localization(self):
        ...
        if time.monotonic() - self.last_loc_reset_time > 0.5:
            ...
            rel_loc_offset[2:3] = (
                -self.body_pos_extend[..., 2].min(dim=0, keepdim=True).values.numpy()
                + 0.015
            )
    ...
    def reset_vision_pro(self):
        ...
        if time.monotonic() - self.last_vp_reset_time > 0.5:
            ...
            quat_delta_buf = quat_from_angle_axis(heading, axis)
            print("DEGREE", heading, "Head DEG", head_heading, "Cur DEG", cur_heading)
            self.vp_delta_rot[:] = quat_mul(quat_delta_buf, self.vp_delta_rot[None, :])[
                0
            ]
            self.vp_delta_rot_mat[:3, :3] = quaternion_to_matrix(
                self.vp_delta_rot.roll(1, dims=0)
            ).numpy()
    ...
    def lowlevel_state_cb(self, msg: LowState):
        ...
        if self.gamepad.A.pressed and USE_DEX:
            if time.monotonic() - self.last_lock_time > 1.0:
                if self.dex_shm_send_data[164] < 1.5:
                    self.dex_shm_send_data[164] = 2.0
                else:
                    self.dex_shm_send_data[164] = 1.0
            self.last_lock_time = time.monotonic()
        ...
        if self.move_by_wireless_remote:
            ...
            if abs(lx) > 0.05 or abs(ly) > 0.05:
                self.loc_offset[0] += lx * 0.008 * np.cos(
                    self.loc_delta_angle
                ) + ly * 0.008 * np.sin(self.loc_delta_angle)
                self.loc_offset[1] += -lx * 0.008 * np.sin(
                    self.loc_delta_angle
        ...
        quat_temp = np.roll(
            np.array(imu_data.quaternion, dtype=np.float32), shift=-1, axis=-1
        )
        self.obs_quat[:] = quat_mul(
            torch.from_numpy(quat_temp)[None, :], imu_correction[None, :]
        )[0].numpy()
        ...
        # motor data
        self.joint_tau[:] = np.array(
            [msg.motor_state[i].tau_est for i in range(HW_DOF)]
        )
        self.joint_pos = [msg.motor_state[i].q for i in range(HW_DOF)]

        self.joint_temperature[:] = np.array(
            [msg.motor_state[i].temperature[1] for i in range(HW_DOF)]
        )
        ...
        factor = 1 - ((self.joint_temperature - 80) / 35).clip(0.0, 1.0)
        ...
        self.over_limit = (
            self.obs_joint_pos
            <= (self.env.joint_limit_lo_ * self.env.soft_dof_pos_limit)
        ).astype(int) + (
            self.obs_joint_pos
            >= (self.env.joint_limit_hi_ * self.env.soft_dof_pos_limit)
        ).astype(
            int
        )
        ...
        if COLLECT_SID_DATA:
            self.sid_data["data_points"].append(
                {
                    "q": self.obs_joint_pos.copy(),
                    "dq": self.obs_joint_vel.copy(),
                    "q_tgt": self.real_q.copy(),
                    "tau": self.joint_tau.copy(),
                    "tau_est": (self.real_q - self.obs_joint_pos) * self.env.p_gains
                    - self.obs_joint_vel * self.env.d_gains,
                    "temp": self.joint_temperature.copy(),
                }
            )
        ...
        self.projected_gravity = (
            quat_rotate_inverse(quat_xyzw, gravity_vector)[0].cpu().numpy()
        )
        ...
        # Joint limit check
        if self.start_policy and (
            ((np.array(self.joint_pos) - np.array(self.env.joint_limit_lo)) < 0).sum()
            > 0
            or (
                (np.array(self.joint_pos) - np.array(self.env.joint_limit_hi)) > 0
            ).sum()
            > 0
        ):
            ...
            print(
                "Low limit Joint index: ",
                np.where(
                    (np.array(self.joint_pos) - np.array(self.env.joint_limit_lo)) < 0
                ),
            )
            print(
                "High limit Joint index: ",
                np.where(
                    (np.array(self.joint_pos) - np.array(self.env.joint_limit_hi)) > 0
                ),
            )
    ...
    def set_motor_position(
        ...
        q = np.clip(
            q,
            self.obs_joint_pos - max_delta_angles,
            self.obs_joint_pos + max_delta_angles,
        )
        for i in range(HW_DOF):
    ...
    def init_policy(self):
        ...
        self.policy = torch.jit.load(
            os.path.join(file_pth, POLICY_PATH), map_location=self.env.device
        )  # 0253 396
        self.policy.to(self.env.device)
        actions = self.policy(
            self.env.obs_buf.detach().reshape(1, -1)
        )  # first inference takes longer time
        # self.policy = None
    ...
    def get_walking_cmd_mask(self):
        ...
        walking_mask = (
            walking_mask
            | (
                self.env.gait_indices.cpu() >= self.control_dt / self.env.cycle_time
            ).numpy()[0]
        )
        walking_mask |= np.logical_or(
            np.abs(self.obs_imu[1]) > 0.05, np.abs(self.obs_imu[0]) > 0.05
        )
        return walking_mask
    ...
    def step_contact_targets(self):
        ...
        self.env.gait_indices = torch.remainder(
            self.env.gait_indices + self.control_dt / cycle_time, 1.0
        )
        if standing_mask:
    ...
    def get_vp_data(self):
        vel_factor = 1.0 if not NO_REF_VEL else 0.0
        left_vel_factor = 1.0
        right_vel_factor = 1.0

        data = self.tv_wrapper.get_data_full()
        if (
            not data is None
            and not FIX_POS
            and (not PLAY_MOTION_DATA or not self.playing)
        ):
            ...
            self.left_wrist_vel[:] = (
                torch.from_numpy(lhand_d[-3]) * vel_factor * left_vel_factor
            )
            self.right_wrist_vel[:] = (
                torch.from_numpy(rhand_d[-3]) * vel_factor * right_vel_factor
            )
            ...
            self.left_hand_rot[:] = matrix_to_quaternion(left_wrist[:3, :3]).roll(
                -1, dims=-1
            )
            self.right_hand_rot[:] = matrix_to_quaternion(right_wrist[:3, :3]).roll(
                -1, dims=-1
            )

            head_delta = (
                my_quat_rotate(self.head_rot[None, :], torch.tensor([[1.0, 0.0, 0.0]]))[
                    0
                ]
                * -0.06
            )
            lhand_delta = (
                my_quat_rotate(
                    self.left_hand_rot[None, :], torch.tensor([[1.0, 0.0, 0.0]])
                )[0]
                * 0.06
            )
            rhand_delta = (
                my_quat_rotate(
                    self.right_hand_rot[None, :], torch.tensor([[1.0, 0.0, 0.0]])
                )[0]
                * 0.06
            )
            ...
            if not self.invalid_hands[0]:
                self.left_wrist_pos = (
                    left_wrist[:3, 3] + lhand_delta + self.vp_delta_pos
                )
            if not self.invalid_hands[1]:
                self.right_wrist_pos = (
                    right_wrist[:3, 3] + rhand_delta + self.vp_delta_pos
                )
            ...
            self.head_vel[:] = 0.0
            self.left_wrist_vel[:] = 0.0
            self.right_wrist_vel[:] = 0.0

            # self.head_rot[:] = torch.tensor([0., 0., 1., 0])
            # self.left_hand_rot[:] = torch.tensor([0., 0., 1., 0])
            # self.right_hand_rot[:] = torch.tensor([0., 0., 1., 0])

            self.head_rot[:] = torch.tensor([0.0, 0.0, 0.0, 1.0])
            self.left_hand_rot[:] = torch.tensor([0.0, 0.0, 0.0, 1.0])
            self.right_hand_rot[:] = torch.tensor([0.0, 0.0, 0.0, 1.0])
    ...
    def launch_vision_pro(self, launch_image_client=True):
        ...
        if VISION_WRAPPER_BACKEND == "vuer":
            ...
            if launch_image_client:
                self.img_client = ImageClient(
                    img_shape, self.img_shm.name, server_address="192.168.123.164"
                )
        ...
        self.tv_wrapper = VisionWrapper(
            backend=VISION_WRAPPER_BACKEND,
            avp_ip=VISION_PRO_IP,
            binocular=False,
            img_shape=img_shape,
    ...
    def compute_task_observation(self) -> np.ndarray:
        ...
        ref_body_pos = (
            torch.cat((self.left_wrist_pos, self.right_wrist_pos, self.head_pos))
            .reshape(1, 1, 3, 3)
            .float()
        )
        ref_body_vel = (
            torch.cat((self.left_wrist_vel, self.right_wrist_vel, self.head_vel))
            .reshape(1, 1, 3, 3)
            .float()
        )
        ...
        body_pos_extend[..., 2] = (
            body_pos_extend[..., 2]
            - torch.min(body_pos_extend[..., 2], dim=0, keepdim=True).values
        )
        ...
        if CLIP_GOAL:
            ...
            if delta_len[0, 0].item() > CLIP_GOAL_DIST:
                ...
                ref_body_pos = (
                    self.body_pos_extend[0:1, :] + head_delta + ref_pos_rel[0, 0]
                ).reshape(1, 1, 3, 3)
        ...
        local_ref_body_rot = quat_mul(
            heading_inv_rot.view(B, 1, 4).repeat(1, 2, 1), wrist_rots.view(B, 2, 4)
        )
        ...
        ref_head_xy[..., 2] = 0.6
        local_ref_body_pos = (
            ref_body_pos.view(B, time_steps, J, 3) - ref_head_xy
        )  # locomotion
    ...
    def compute_observations(self):
        ...
        history_to_be_append = self.env.trajectories[
            0 : self.env.obs_context_len * LOG_STEP_LEN
        ].copy()
        ...
        while rclpy.ok() or NO_ROS:
            ...
            if self.stop:
                ...
                if COLLECT_MOTION_DATA:
                    ...
                    torch.save(
                        self.motion_data,
                        f'./rec_motion/motion_data_{now.strftime("%m_%d_%H_%M_%S")}.pkl',
                    )
                    print("Motion Data Saved")
                ...
                if COLLECT_SID_DATA:
                    ...
                    now = datetime.datetime.now()
                    with open(
                        f'./sys_id/sid_data_{now.strftime("%m_%d_%H_%M_%S")}.pkl', "wb"
                    ) as f:
            ...
            # spin stuff
            while (
                self.control_dt > time.monotonic() - loop_start_time
            ):  # 0.012473  0.019963
                # rclpy.spin_once(self, timeout_sec=0.0001)
                time.sleep(
                    max(
                        0.0,
                        self.control_dt - (time.monotonic() - loop_start_time) - 0.001,
                    )
                )
                pass
            loop_start_time = time.monotonic()
            ...
            angles = (
                raw_actions_np * self.env.scale_action + self.env.default_dof_pos_np
            )
            ...
            self.env.record_trajectory(
                dof,
                dof_vel,
                base_ang_vel,
                base_gravity,
                torch.from_numpy(self.prev_action),
                task_obs,
            )
            ...
            if self.recording:
                ...
                if USE_DEX:
                    cur_motion = torch.cat(
                        [
                            cur_motion,
                            torch.from_numpy(self.dex_shm_recv_data[:14])
                            .float()
                            .clone(),
                        ]
                    )
