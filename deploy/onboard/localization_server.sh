...
# Pane 0: localization
tmux send-keys -t lio:0.0 'source devel/setup.zsh && roslaunch fast_lio_localization localization_mid360.launch' C-m
sleep 2

# Split pane 0 horizontally -> creates pane 1
tmux split-window -h -t lio:0.0
tmux send-keys -t lio:0.1 'source devel/setup.zsh && roslaunch livox_ros_driver2 msg_MID360.launch' C-m
sleep 2

# Split the original left pane horizontally again -> creates pane 2 (three columns)
tmux split-window -h -t lio:0.0
tmux send-keys -t lio:0.2 'python /catkin_ws/deploy_onboard/pos_server.py' C-m
sleep 1
