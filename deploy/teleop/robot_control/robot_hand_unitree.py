...
class Gripper_Controller:
    def __init__(
        ...
        if filter:
            self.smooth_filter = WeightedMovingFilter(
                np.array([0.5, 0.3, 0.2]), Gripper_Num_Motors
            )
        else:
        ...
        self.GripperState_subscriber = ChannelSubscriber(
            kTopicGripperState, MotorStates_
        )
        ...
        # initialize subscribe thread
        self.subscribe_state_thread = threading.Thread(
            target=self._subscribe_gripper_state
        )
        self.subscribe_state_thread.daemon = True
        self.subscribe_state_thread.start()
    ...
    def control_thread(
        ...
        self.gripper_msg.cmds = [
            unitree_go_msg_dds__MotorCmd_() for _ in range(len(Gripper_JointIndex))
        ]
            ...
            while self.running:
                ...
                if not np.all(
                    ...
                    left_euclidean_distance = np.linalg.norm(
                        left_hand_mat[unitree_gripper_indices[1]]
                        - left_hand_mat[unitree_gripper_indices[0]]
                    )
                    right_euclidean_distance = np.linalg.norm(
                        right_hand_mat[unitree_gripper_indices[1]]
                        - right_hand_mat[unitree_gripper_indices[0]]
                    )
                ...
                dual_gripper_action = np.array(
                    [right_actual_action, left_actual_action]
                )
                ...
                if dual_gripper_state_out and dual_gripper_action_out:
                    with dual_hand_data_lock:
                        dual_gripper_state_out[:] = dual_gripper_state - np.array(
                            [RIGHT_MAPPED_MIN, LEFT_MAPPED_MIN]
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gripper", dest="dex", action="store_false", help="Use gripper"
    )
    ...
    # television and arm
    tv_wrapper = TeleVision(BINOCULAR, tv_img_shape, img_shm.name)
    ...
    if args.dex:
        ...
        dual_hand_state_array = Array(
            "d", 14, lock=False
        )  # current left, right hand state(14) data.
        dual_hand_action_array = Array(
            "d", 14, lock=False
        )  # current left, right hand action(14) data.
