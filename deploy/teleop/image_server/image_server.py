...
class RealSenseCamera(object):
    ...
    def get_frame(self):
        ...
        device_capture_time = color_frame.get_frame_metadata(
        ...
        depth_image = (
            np.asanyarray(depth_frame.get_data()) if self.enable_depth else None
        )
...
class ImageServer:
    def __init__(self, config, port=5555, Unit_Test=False):
        ...
        self.head_image_shape = config.get(
        ...
        self.wrist_image_shape = config.get(
        ...
        if self.head_camera_type == "opencv":
            for device_id in self.head_camera_id_numbers:
                camera = OpenCVCamera(
                    device_id=device_id, img_shape=self.head_image_shape, fps=self.fps
                )
            ...
            print(
                f"[Image Server] Unsupported head_camera_type: {self.head_camera_type}"
            )
        ...
        if self.wrist_camera_type and self.wrist_camera_id_numbers:
            if self.wrist_camera_type == "opencv":
                ...
                print(
                    f"[Image Server] Unsupported wrist_camera_type: {self.wrist_camera_type}"
                )
                ...
                if self.wrist_cameras:
                    ...
                    for cam in self.wrist_cameras:
                        if self.wrist_camera_type == "opencv":
                            ...
                            if color_image is None:
                                print(
                                    "[Image Server] Wrist camera frame read is error."
                                )
                                break
                        elif self.wrist_camera_type == "realsense":
                            color_image, depth_iamge = cam.get_frame()
