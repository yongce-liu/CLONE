...
def publish_downsampled_pcd(pcd_file_path, publish_topic, map_topic, resolution):
    ...
    R = pcd.get_rotation_matrix_from_xyz(
        (0.0 / 180 * math.pi, -3.0 / 180 * math.pi, 0)
    )  # bigai-15-12-23---((2.0/180*math.pi, -3.0/180*math.pi, 0))     # bigai_15-11-12_3----(-1.5/180*math.pi, -6/180*math.pi, 0)     bigai_15_11-13----((-0.5/180*math.pi, -6/180*math.pi, 0))
    pcd.rotate(R, center=(0, 0, 0))
...
def image_to_occupancy_grid(img, min_x, min_y, resolution):
    ...
    map_metadata.resolution = (
        resolution  # Actual size (meters) represented by each pixel
    )
